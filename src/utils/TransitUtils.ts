...
/**
 * Rileva la targa a seconda del tipo di gate:
 *  - STANDARD: OCR sull'immagine del veicolo (imagePath)
 *  - SMART: lettura da JSON (jsonPath)
 */
export async function detectPlateForGate(gate: any, vehicle: any): Promise<string> {
  let detectedPlate: string | null = null;

  if (gate.type === GateType.STANDARD) {
    if (!vehicle.imagePath) {
      throw new ValidationError(
        "Vehicle senza imagePath per gate di tipo STANDARD"
      );
    }
    ...
    if (!vehicle.jsonPath) {
      throw new ValidationError(
        "Vehicle senza jsonPath per gate di tipo SMART"
      );
    }

    const fullJsonPath = path.resolve(process.cwd(), vehicle.jsonPath);
    const jsonRaw = fs.readFileSync(fullJsonPath, "utf-8");
    const payload = JSON.parse(jsonRaw);

    detectedPlate = payload.plate || payload.licensePlate || null;
  } else {
    throw new ValidationError(
      "Tipo gate non supportato per la rilevazione targa"
    );
  }

  if (!detectedPlate) {
    throw new ValidationError("Impossibile determinare la targa rilevata");
  }
